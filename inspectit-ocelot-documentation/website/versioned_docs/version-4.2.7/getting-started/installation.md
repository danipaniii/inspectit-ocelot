...
```bash
$ java -jar inspectit-ocelot-agent-4.2.7.jar <PID> [<AGENT_ARGUMENTS>]
```
...
```bash
$ java -jar inspectit-ocelot-agent-4.2.7.jar 1337 '{"inspectit":{"service-name":"my-agent"}}'
```
...
```bash
$ ./jattach.sh <PID> load instrument false /path/to/inspectit-ocelot-agent-4.2.7.jar='{"inspectit.service-name" : "MyService"}'
```
